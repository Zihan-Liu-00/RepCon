...
def collate(samples):
    graphs, sequences ,labels = map(list, zip(*samples))
    if labels[0].dtype == torch.int64:
        return dgl.batch(graphs).to(device), torch.stack(sequences).to(device), torch.tensor(labels).to(device)
    else:
        return dgl.batch(graphs).to(device), torch.stack(sequences).to(device), torch.tensor(labels).unsqueeze(1).to(device)

def make_dgl_dataset(args):
    for split in ['train','valid','test']:
        df = pd.read_csv('seq dataset/{}/{}.csv'.format(args.dataset,split))
        feat = np.array(df["Feature"])
        label = torch.Tensor(np.array(df["Label"])).unsqueeze(1)
        dgl_graph = make_data(feat)
        dgl.save_graphs('graph dataset/{}/{}.bin'.format(args.dataset,split), dgl_graph, {'labels': label})
